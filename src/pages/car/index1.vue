<template>
	<div class="car">
		...
		<div class="car-list">
			...
			<ul class="shop-goods">
				<!-- 购物车列表详细 -->
				<li class="shop-goods-list" v-for="(item,index) in shopGoodstList" :key="index" >
					<!-- 店铺名字 -->
					<div class="shop-title">
						<!-- 选择框 -->
						<div class="check-box" >
							<input type="checkbox"  v-model="shopallState"/>
					...
					<!-- 商品清单 -->
					<ul class="goods-list">
						<li class="goods-list-li" v-for="(item,levindex) in item.goodslist" :key="levindex" :style='item.moveStyle'
						v-swipeleft = "{fn:swipeleft,index:levindex}" v-swiperight = "{fn:swiperight,index:levindex}">
							<!-- 选择框 -->
							<div class="check-box">
		...
		<div class="manager-nav">
			<!-- 选择框 -->
			<div class="all-check">
				<div class="check-box" @click="shopcheckAll()" v-model="allState" >
					<input type="checkbox" />
					<label class="checkbox-label"></label>
				</div>
...
// 		},
		...
		swipeleft(s,e) {
		 ...
		 // setMOve
        _setMove(index = ''){
			console.log("移动setmove")
			console.log(index)
            let that = this;
            for(let i = 0; i < that.shopGoodstList.length; i++){
                that.shopGoodstList[i].moveStyle = "transform:translateX(0rem)"
            }
            if(index !== ''){
				console.log(that.shopGoodstList[index])
		...
		// 店铺选择框操作
		shopcheckAll(){
			// this.shopallState = !this.shopallState;
			if(this.allState){
